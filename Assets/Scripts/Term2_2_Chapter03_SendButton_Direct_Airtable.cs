...
public class Term2_2_Chapter03_SendButton_Direct_Airtable : MonoBehaviour, IPointerClickHandler
{
    // Airtable の Base ID
    ...
    IEnumerator PostPointData()
    {
        ...
        // データを設定
        // 現在のポイントを得る
        requestDataRecord.fields.Point = GameObject.Find("ClickPart").GetComponent<Term2_2_Chapter03_ClickPart>().currentPoint;
        // 自分の名前を登録
        // 固定値でなく InputField から取得する
        requestDataRecord.fields.Name = GameObject.Find("InputField").GetComponent<InputField>().text;
        // records に加える
        ...
        switch (request.result)
        {
            ...
            case UnityWebRequest.Result.Success:
                ...
                RequestData resultResponse = JsonUtility.FromJson<RequestData>(request.downloadHandler.text);

                // RankingMessage で最新データを取得
                // GetDataCore を動かす
                GameObject.Find("RankingMessage").GetComponent<Term2_2_Chapter03_RankingMessage_Direct_Airtable>().GetDataCore();
